...
import { DEFAULT_TAXONOMY, isContentType, type Category, type ContentType, type TaxonomyFilter } from '@/lib/taxonomy'
import {
...
function DiscoverPageContent() {
  ...
  const generatedTitles = useMemo(
    () => generatedStories.map((story) => story.title),
    [generatedStories]
  )
  ...
  const generatedHeadingKey = isNews ? 'searchGeneratedBriefings' : 'searchGeneratedEpisodes'
  ...
  useEffect(() => {
    ...
    const applyPersistedGeo = () => {
      ...
      setDraftFilter((prev) => ({ ...prev, ...geo }))
      setFilterState((prev) => {
        if (prev.contentType !== 'News') return prev
        const next = { ...prev, ...geo }
        setHasLoaded(false)
        return next
      })
