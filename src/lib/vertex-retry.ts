...
export async function fetchWithImagenRetry(
  ...
  for (let attempt = 1; attempt <= IMAGEN_MAX_ATTEMPTS; attempt++) {
    try {
      const res = await fetchFn(url, init)
      lastRes = res
      if (res.ok || !isTransientImagenHttpStatus(res.status) || attempt >= IMAGEN_MAX_ATTEMPTS) {
        return res
      }

      const delay = imagenRetryDelayMs(res, attempt)
      if (res.status === 429) {
        console.warn(
          `[vertex] imagen rate limited; retrying in ${Math.round(delay / 1000)}s (attempt ${attempt}/${IMAGEN_MAX_ATTEMPTS})`
        )
      } else {
        console.warn(
          `[vertex] imagen predict ${res.status}; retrying in ${Math.round(delay / 1000)}s (attempt ${attempt}/${IMAGEN_MAX_ATTEMPTS})`
        )
      }
      await sleep(delay)
    } catch (error) {
      if (attempt >= IMAGEN_MAX_ATTEMPTS) throw error
      const delay = imagenRetryDelayMs(new Response(null, { status: 503 }), attempt)
      console.warn(
        `[vertex] imagen predict error; retrying in ${Math.round(delay / 1000)}s (attempt ${attempt}/${IMAGEN_MAX_ATTEMPTS})`
      )
      await sleep(delay)
    }
  }
  ...
  if (!lastRes) {
    throw new Error('imagen_fetch_failed')
  }
  return lastRes
}
